<?php 
//use Spout\Reader\ReaderFactory;
		use Box\Spout\Reader\ReaderFactory;
		use Box\Spout\Common\Type;
		 
		// Include Spout library 
		require_once 'Spout/Autoloader/autoload.php';
	//if(isset($_REQUEST['upload'])){

		if (!empty($_FILES['files']['name'])){   
		    // Get File extension eg. 'xlsx' to check file is excel sheet
		    $pathinfo = pathinfo($_FILES["files"]["name"]);
		    echo "path".$_FILES["files"]["name"];
		    // check file has extension xlsx, xls and also check file is not empty
		   if (($pathinfo['extension'] == 'xlsx' || $pathinfo['extension'] == 'xls') && ($_FILES['files']['size'] > 0) ) {
		         // Temporary file name
		        $inputFileName = $_FILES['files']['tmp_name']; 
		        // Read excel file by using ReadFactory object.
		        $reader = ReaderFactory::create(Type::XLSX);
		        // Open file
		        $reader->open($inputFileName);
		        $count = 1;
		        $rows = array(); 
		        // Number of sheet in excel file
		        foreach ($reader->getSheetIterator() as $sheet) {
		            // Number of Rows in Excel sheet
		            foreach ($sheet->getRowIterator() as $row) {
		                // It reads data after header. In the my excel sheet, 
		                // header is in the first row. 
		                if ($count > 1) { 
		                	$data['block_id'] = $row[0];
		                    $data['department_id'] = $row[1];
		                    $data['name']=$row[2];
		                    $data['address']=$row[3];
		                    $data['date_of_birth']=$row[4];
		                    $data['phone']=$row[5];
		                    $data['email']=$row[6];
		                    $data['password']=$row[7];
		                    // Push all data into array to be insert as  batch into MySql database.  
		                    array_push($rows, $data);
		                }
		                $count++;
		            }          
		        }
		        include "Controllers/config.php";
				foreach($rows as $r){
					$c1 = $r['block_id'];
					$c2 = $r['department_id'];		

					$c3 = $r['name'];
					$c4 = $r['address'];		
					$c5 = $r['date_of_birth'];		
					$c6 = $r['phone'];		
					$c7 = $r['email'];		
					$c8 = $r['password'];		

					$k =mysqli_query($con,"INSERT INTO hods  VALUES('','$c1','$c2','$c3','$c4','$c5','$c6','$c7','$c8') ");
		            if ($k<=0)
		            {
		                echo mysqli_error($con);
		                die();
		            }
				}
		        // Close excel filew+
		        $reader->close();
		        echo "<script>window.location.assign('hod.php?msg=HODs added successfully.')</script>";
				
		 	}
		      else {
		 
		        // echo $pathinfo['extension'].$_FILES['files']['size']."Gaurav";
		        echo "<script>window.location.assign('hod.php?msg=Please Select Valid Excel File containing HODs details.')</script>";
				
		     }
	 
	} 

	elseif(isset($_REQUEST['submit'])){
		include "Controllers/config.php";
		$block=$_REQUEST['block'];
		$department=$_REQUEST['department'];
		$name=$_REQUEST['name'];
		$address=$_REQUEST['address'];
		$phone=$_REQUEST['phone'];
		
		$email=$_REQUEST['email'];
		$password=$_REQUEST['password'];
		$dob=$_REQUEST['dob']	;																																											
		

		$q="insert into hods values('','$block','$department','$name','$address','$dob','$phone','$email','$password')";
		$res=mysqli_query($con,$q);
		if($res>0){
			
		    echo "<script>window.location.assign('hod.php?msg=HOD added successfully.')</script>";

		}
		
	}
	else{
		echo "<script>window.location.assign('hod.php')</script>";

	
	}	
?>